...
import { classifyRateLimit, retryAfterSeconds, humanizeWait, rateLimitMessage } from 'ai-ration';
...
test('requests-per-day is treated as daily too', () => {
  assert.equal(classifyRateLimit('Rate limit reached on requests per day (RPD): Limit 50'), 'daily');
});
