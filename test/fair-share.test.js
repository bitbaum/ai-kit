...
import { fairShare, utcDayElapsed, utcDayKey, DAY_SECONDS, DEFAULT_BURST } from 'ai-ration';

const TURN = 20_000;
const NOON = 0.5;

const ask = (over) =>
  fairShare({ dayCapacityTokens: 8 * TURN, activeUsers: 2, userSpentTokens: 0, costTokens: TURN, dayElapsed: NOON, ...over });
...
test('hostile inputs cannot produce a division by zero or a negative allowance', () => {
  for (const activeUsers of [0, -3, NaN]) {
    ...
    assert.ok(Number.isFinite(d.shareTokens), `shareTokens not finite for activeUsers=${activeUsers}`);
  ...
  assert.ok(future.allowanceTokens <= future.shareTokens, 'a clamped day must not exceed the share');
});
